...
def get_template_sdf(template_list, dataset_infos):
    ...
    for i in range(len(template_list)):
        mol = Molecule(atom_types=dense_data.X[i], charges=dense_data.charges[i],
                       ...
                       template_idx=template_list[i].idx,
                       centroid_pos=template_list[i].centroid_pos).rdkit_mol
        # pos = np.array(mol.GetConformers()[0].GetPositions())
        # new_pos = pos + template_list[i].centroid_pos
        # conf = Chem.Conformer(mol.GetNumAtoms())
        # for atom_idx in range(mol.GetNumAtoms()):
        #     conf.SetAtomPosition(atom_idx, Point3D(new_pos[atom_idx, 0],new_pos[atom_idx, 1],new_pos[atom_idx, 2]))
